#!/bin/bash
# =====================================================================
# Disk Offline Performance Analyzer
# =====================================================================
...
init_logger "disk_analyzer" $LOG_LEVEL "${LOGS_DIR}/disk_analyzer.log"

# Disk performance analysis function
analyze_disk_performance() {
    ...
    log_info "Starting Disk performance data analysis: $csv_file"
    ...
    log_info "Disk performance analysis completed"
}
...
analyze_device_performance() {
    ...
    tail -n +2 "$csv_file" | while IFS=',' read -r line_data; do
        ...
        local warning_util_threshold=$(awk "BEGIN {printf \"%.2f\", ${BOTTLENECK_DISK_UTIL_THRESHOLD:-90} * 0.8}")
        ...
        local warning_latency_threshold=$(awk "BEGIN {printf \"%.2f\", ${BOTTLENECK_DISK_LATENCY_THRESHOLD:-50} * 0.4}")
...
# Main function
main() {
    log_info "Disk performance analyzer started"
    ...
    if [[ -z "$csv_file" ]]; then
        ...
        echo ""
        echo "Analyze Disk performance data in CSV file generated by unified monitor"
    ...
    analyze_disk_performance "$csv_file"
}
