...
# generate_expected_csv_header uses this registry to avoid duplicate disk header definitions.
...
generate_expected_csv_header() {
    # Basic section header generated through csv_schema_registry when available.
    ...
    # Device header is generated by csv_schema_registry and aligned with iostat_collector.
    # Provider-normalized disk fields use normalized_iops/normalized_throughput_mibs.
    # provider comes from get_provider_name; it remains part of the registry interface.
    ...
    # Actual header appends cloud_provider as the final column.
    # Existing limitation: this expected header does not include cgroup_header fields.
    # Keep the cloud_provider column aligned with current runtime output.
    ...
    if [[ -d "$logs_dir" ]]; then
        ...
        if [[ -n "$block_csv" ]]; then
            ...
            if declare -F csv_registry_block_csv_header >/dev/null 2>&1; then
                block_header="$(csv_registry_block_csv_header)"
            else
                block_header="timestamp,local_block_height,mainnet_block_height,block_height_diff,local_health,mainnet_health,data_loss,sync_mode,sync_status,lag_value,lag_unit,freshness_gap_seconds,probe_error"
            fi
...
validate_shared_memory_files() {
    ...
    if [[ -f "${LATEST_METRICS_FILE:-${MEMORY_SHARE_DIR}/latest_metrics.json}" ]]; then
        validate_json_file "${LATEST_METRICS_FILE:-${MEMORY_SHARE_DIR}/latest_metrics.json}" "Core metrics" "timestamp cpu_usage memory_usage"
    else
    ...
    if [[ -f "${UNIFIED_METRICS_FILE:-${MEMORY_SHARE_DIR}/unified_metrics.json}" ]]; then
        validate_json_file "${UNIFIED_METRICS_FILE:-${MEMORY_SHARE_DIR}/unified_metrics.json}" "Detailed metrics" "timestamp cpu_usage memory_usage detailed_data"
    else
    ...
    local block_height_cache_file="${BLOCK_HEIGHT_CACHE_FILE:-${MEMORY_SHARE_DIR}/block_height_monitor_cache.json}"
    if [[ -f "$block_height_cache_file" ]]; then
        local block_cache_required="timestamp local_block_height mainnet_block_height sync_mode sync_status"
        if declare -F csv_registry_block_cache_required_keys >/dev/null 2>&1; then
            block_cache_required="$(csv_registry_block_cache_required_keys)"
        fi
        validate_json_file "$block_height_cache_file" "Block height cache" "$block_cache_required"
    else
    ...
    [[ -f "${BOTTLENECK_STATUS_FILE:-${MEMORY_SHARE_DIR}/bottleneck_status.json}" ]] && validate_json_file "${BOTTLENECK_STATUS_FILE:-${MEMORY_SHARE_DIR}/bottleneck_status.json}" "Bottleneck status" ""
    [[ -f "${UNIFIED_EVENTS_FILE:-${MEMORY_SHARE_DIR}/unified_events.json}" ]] && validate_json_file "${UNIFIED_EVENTS_FILE:-${MEMORY_SHARE_DIR}/unified_events.json}" "Unified events" ""
}
...
validate_data_consistency() {
    ...
    if [[ -n "$perf_csv" && -f "${LATEST_METRICS_FILE:-${MEMORY_SHARE_DIR}/latest_metrics.json}" ]]; then
        check_count
        local csv_last_timestamp=$(tail -1 "$perf_csv" | cut -d',' -f1)
        local json_timestamp=$(jq -r '.timestamp' "${LATEST_METRICS_FILE:-${MEMORY_SHARE_DIR}/latest_metrics.json}" 2>/dev/null)
