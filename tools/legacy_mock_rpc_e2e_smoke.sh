#!/usr/bin/env bash
# =====================================================================
# legacy_mock_rpc_e2e_smoke.sh - legacy mock-RPC smoke harness
# =====================================================================
# Proves the entire pipeline works from "mock node + workload" to
# "HTML report file exists".
#
# DEFAULT BEHAVIOR (no args, no env):
#   - starts legacy_mock_rpc_server on localhost:18545 (ethereum chain default)
...
#                             #   chain-template consistency). No behavior
...
# "chain template exists but legacy_mock_rpc_server has no matching handler"
...
if [[ -n "$CHAIN_CONFIG" ]]; then
  ...
  # legacy_mock_rpc_server.py CHAIN_HANDLERS dict is the final arbiter via boot
...
python3 "${REPO_ROOT}/tools/legacy_mock_rpc_server.py" \
...
if [[ "$FAILURES" -eq 0 ]]; then
  echo "legacy_mock_rpc_e2e_smoke: PASS" >&2
  echo "═══════════════════════════════════════════════════════════════" >&2
  exit 0
else
  echo "legacy_mock_rpc_e2e_smoke: FAIL (${FAILURES} assertion failures)" >&2
  echo "═══════════════════════════════════════════════════════════════" >&2
  exit 1
fi
