#!/usr/bin/env python3
"""Legacy mock RPC server for early blockchain-node-benchmark e2e tests.

The current 36-chain closed-loop path uses tools/fake-node. This helper is
kept only for legacy smoke tests that exercise the original JSON-RPC baseline.

Supports HTTP POST (JSON-RPC 2.0) + WebSocket subscribe for the legacy baseline:
- Solana (8899)
- EVM 5 chains: Ethereum / BSC / Base / Polygon / Scroll
- Starknet
- Sui

Source of truth for RPC methods: config/chains/<name>.json.

Usage:
    python3 legacy_mock_rpc_server.py --port 8899 --chain solana
    python3 legacy_mock_rpc_server.py --port 8545 --chain ethereum --latency-ms 5
    python3 legacy_mock_rpc_server.py --port 8899 --chain solana --ws-port 8900
...
def handle_evm(method: str, params: List[Any], chain: str = "ethereum") -> Any:
    ...
    if method == "eth_getBlockByHash":
        # Forward chain context to the recursive call so future
        # chain-specific block fields do not fall back to ethereum defaults.
    ...
    # Chain-specific: L2 chain-specific methods (zksync-era / linea / arbitrum / optimism)
...
CHAIN_HANDLERS = {
    ...
    "sui": handle_sui,
    # Chain-specific: EVM-compat 5 chains (full EVM JSON-RPC + chain-specific methods
