#!/usr/bin/env bash
# gcp_gce_setup.sh — One-shot GCE bastion setup for blockchain-node-benchmark
#
# Creates a fresh GCE instance with everything needed to build collector images
# and deploy to GKE. Run this BEFORE gcp_gke_setup.sh.
#
# Produces:
#   - GCE e2-standard-2 in us-central1-f, Debian 12 (bookworm)
#   - Service account bound to instance (scopes: cloud-platform)
...
#
# Usage:
#   PROJECT=my-gcp-project bash scripts/gcp_gce_setup.sh
#
# Cleanup when done:
...
SA_NAME="${SA_NAME:-blockchain-bench-bot}"
...
if ! gcloud iam service-accounts describe "$SA_EMAIL" --project="$PROJECT" &>/dev/null; then
  ...
  echo "      --display-name='Blockchain benchmark bot'"
  exit 1
fi
ok "SA exists"
