...
| Q4-7 | **per-method 归因算法** | **加权 group_by + 秒级时间窗**;~~**权重源 = 公开资料先配粗粒度**(`analysis-notes/research_notes/01-06` 各 method "典型延迟量级" → 映射 1/10/100 三档)~~ **【2026-06-02 代码校准: 实现已演进, 见下】**;**后期实际使用根据真实压测数据迭代调整**(非阻塞决策);PoC 撤销条件:ground truth 误差 > 20% 才升级回归算法。<br>**⚠️ 2026-06-02 代码实查更正(代码新于本决策, 以代码为准)**: `analysis/per_method_attribution.py:14-16` 实现的权重 = **实测频次权重**(秒级时间窗内该 method 真实请求次数占比), 即 `method_weight(method,t) = count(method in [t,t+1)) / total_count(in [t,t+1))`, `method_cpu = total_cpu(t) * method_weight`。**不是预设 1/10/100 档**, 而是直接用 proxy 实测流量频次 — 这正是 Q4-7 原本规划的"后期按真实压测数据迭代"已落地。`__unmatched__`(extractor 未匹配)记录跳过不归因。 |
...
| Q4-9 | **sink 默认格式** | **默认 CSV** + ~~字段最小集 6 列(`timestamp, method, req_bytes, resp_bytes, latency_ms, status`)~~ **【2026-06-02 代码校准: 实际 9 列, 见下】**;sink 抽象层支持 JSONL/Parquet 切换(环境变量 `PROXY_SINK_FORMAT`);无强撤销条件,日志体积超 100GB/天再评估。<br>**⚠️ 2026-06-02 代码实查更正(代码新于本决策, 以代码为准)**: `tools/proxy/internal/sink/sink.go:41-42` 实际写 **9 列** = `timestamp_ns, method_name, protocol, request_id, batch_idx, status_code, latency_ms, upstream, client_addr`。**与原 6 列规划差异**: 无 `req_bytes`/`resp_bytes`(归因不需要请求/响应字节数, 用 method 频次即可); 多了 `protocol`(family 判别)/`request_id`+`batch_idx`(批量请求拆分)/`upstream`(转发目标)/`client_addr`。**proxy 有意不缓冲响应 body**(handler.go:103, 大 response 直接 stream, 保 Q4-8 性能)→ 响应结构体记录另走默认关的旁路开关(见 `analysis-notes/rpc-method-abstraction-design.md` §5.6)。 |
