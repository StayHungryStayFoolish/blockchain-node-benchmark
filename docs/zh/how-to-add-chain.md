...
5. 在 `tools/fake-node/configs/` 对应 family YAML 中增加 fixture 映射。
6. 录制该 method 自己的 fixture，并运行 coverage/runtime probes。
...
每个会进入 `single` 或 `mixed_weighted` 的 method 都必须有 `param_formats`。
...
如果需要新参数格式，例如特殊地址编码、特殊 body 模板、特殊 query 参数，先确认现有 adapter 是否支持。支持则只改 JSON；不支持则要扩展 `tools/chain_adapters/<family>.py`。
...
1. 在 `config/chains/<chain>.json` 增加 `param_formats.<method>`。
2. 如果 method 要参与 mixed，加入 `rpc_methods.mixed` 和 `mixed_weighted`。
3. REST method 还要增加 `_meta.rest_paths.<method>`。
4. 在 fake-node family YAML 增加 method 到 fixture 文件名的映射。
5. 准备真实样本参数。
6. 重新录制 fixture。
7. 跑真实性、覆盖率和 runtime probe。
...
- 参数需要链特有编码，现有 `param_formats` 不支持。
...
### 错误 2：`mixed_weighted` 里有 method，但 `param_formats` 缺失

target generator 无法构造请求。每个 method 都必须有参数格式。
...
- [ ] `single` 和 `mixed_weighted` 中所有 method 都有 `param_formats`。
