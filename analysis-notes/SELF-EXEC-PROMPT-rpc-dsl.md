...
> ⚠️⚠️ **2026-06-04 状态降级标记(防混乱, 必读)**:
> - 本文 **RPC 事实/行号/架构已部分过时**, **执行/改代码绝不以本文为依据** —— 唯一权威 = `REFACTOR-SSOT.md` + `REFACTOR-TASK-STATUS.md`。
> - 本文现仅作 **"搬运 SSOT 的原料索引"**(它的全功能点落点表是搬 SSOT 时的指引线索), **待 SSOT 把所有单元(S2/S3/S0/决策/约束)搬完并确认完全覆盖后, 本文即删除**(原料用完才删, 见用户 2026-06-04 指示)。
> - **已知过时点(勿信)**: 下方落点表 A1 `create_adapter L673`(实证 L665-668)、A2 `L729/L818`(实证 L204/313/335)、**A3 `target_generator.sh 取值`(架构已否决: 取值在 build_vegeta_target, 见 SSOT S1 三层职责)**; 其余行号未逐个 grep 实证, 搬 SSOT 时必 grep 重核。
> - skill 引用本文(5处)= 引"续命prompt写法 + 碎片化反例"的方法论案例(skill 正文自足), 删本文不影响 skill 可读性。
>
