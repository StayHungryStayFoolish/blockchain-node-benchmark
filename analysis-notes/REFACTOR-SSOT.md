...
- **🔴 bash 侧两个死函数(S2 老路退役落点, grep caller 实证)**: (a) `generate_rpc_json`(target_generator.sh L67)= **0 caller 死代码**(注释 L68-69 自承 legacy wrapper, body L74 也只是调 python cli.py); (b) `get_param_format_from_json`(config_loader.sh L683)= **0 真 caller 死代码**(唯一命中 cli.py L31 是注释引用 `Mirrors config_loader.sh:600 get_param_format_from_json()`, 非调用; 且该注释行号过时说 L600 实 L683)。两者 + export 行(config_loader.sh L718)在 S2 接 param_spec 后应清理。对照: `get_current_rpc_methods`(L668) 仍 4 live caller(target_generator.sh L17/107/309/310) **保留**。
