...
- **批3 推广 5 family + 老枚举退役**: rest/tendermint/substrate/bitcoin/hedera 各接 param_spec 构造器; 6 adapter 老 `_build_params` if-else 删除(成 dead 后删, 成因d); cli.py `_get_param_format` 退役(resolve_param_spec 接管)。**批3 L1 门**: F2 L2 占位基线下降(真值池接入后) + R0 过 + ci_smoke 19/0。
  ...
  - **🔴 批3b REST 声明式构造详细设计(2026-06-05 token-level 实证 rest_paths 全貌)**:
    ...
    - **🔴 批3b 删 _resolve_path + 自检的真问题(2026-06-05 用户连续两次反问"是否 token-level 确认"逼出, 第一次我仍走捷径用 grep 自证, 第二次逐行读+实跑才看清, 含我归因错误)**:
      - **ton 归因修正(我之前推断错)**: 我曾说"删 _resolve_path 导致 ton body 漏构造"——**实跑证伪**: ton runGetMethod/getAddressBalance 是 param_format 自然语言枚举(`{address,method...}`)无 PRESET → **resolve_param_spec 阶段就 fail-fast**, 根本走不到 rest.py body 构造, 与删 _resolve_path 无关。修复=补 ton PRESET 或显式 param_spec(批3b收尾)。
      - **🔴 cardano POST_ASSET_INFO 我自己写的真 bug(逐行读 rest.py L139-142 + 实跑发现)**: body_template `{policy}`/`{asset_name}` 两个不同语义占位, rest.py `or inputs.get("account")` 兜底 → 实跑 body=`[["addr1xyz","addr1xyz"]]`(policy 和 asset_name 都填同一 account 地址=错值! policy_id 28字节资产标识≠地址, 且两者不该相同)。这是我图省事的兜底引入的语义 bug, 第一次"自检"没发现(只看 PRESET 有无 body_template, 没跑出 body 真实内容)。修复=business 池区分 policy/asset_name(批4)或显式声明。
      - cardano 4 个 body method 的 config.rest_paths.body(用 {address})成 dead 双源(确认): PRESET body_template 接管, config body 不再读 → 批3b收尾清理。
      - **元教训**: ① 删码前等价性核对我用 grep 自证不算 token-level(用户探针①"凭挖出东西自证") ② 判断"漏/dead"前必实跑看真实行为不推断(ton 归因错就是推断) ③ 自己刚写的代码必逐行通读+真值实跑(cardano bug 是自己写的 or account 兜底, 自检没跑 body 内容漏掉) ④ 门绿+单点grep ≠ 逻辑对。
    - **🔴 批3b 收尾 4 链真实归类(2026-06-05 实跑失败原因 + 官方调研, token-level)**:
      - **near(query dispatcher)**: family=jsonrpc, single=`query`, param_format=`query_dispatcher_request_type`。near query 是 dispatcher: **params 是 dict(非 list)**, 含 `request_type` 字段判别查询类型。view_account: `params={request_type:'view_account', finality:'final', account_id:'<addr>'}`。→ 补 jsonrpc_dict PRESET(transport jsonrpc_dict, fields request_type=literal/finality=literal/account_id=account)。**官方结构待 token-level 复核**(子agent调研未完成, NEAR docs/api/rpc/contracts)。
      - **ton(自然语言枚举→显式结构)**: family=rest。官方 toncenter v2 OpenAPI 实证: getAddressBalance/getAddressInformation/getTransactions/lookupBlock=GET query(address friendly base64url|raw), **仅 runGetMethod=POST body** {address, method:'seqno', stack:[]}。getTransactions 有 limit/lt/hash/to_lt/archival query 参数; lookupBlock shard=十进制字符串 int64(易错), seqno/lt/unixtime 三选一。→ 补 rest_query(getAddressBalance 等)+ rest_body(runGetMethod) PRESET, 占位 address→account。
      - **hedera(REST 侧缺 param_format)**: family=hedera_dual, REST method `GET /api/v1/accounts/{addr}` **param_format=无** → resolve_param_spec fail-fast。修=给 hedera REST method 配 param_format(path_addr 等)。hedera_dual adapter per-request 路由已有(_is_jsonrpc_method 分 eth_*→jsonrpc / REST path→rest), 只缺 config 配 param_format。
      - **🔴 tron(归 S3.7 架构, 非简单补配置)**: family=jsonrpc **但 method 是混协议**(`/wallet/*` REST POST body + `eth_blockNumber` jsonrpc)。SSOT S3.7 已定: tron 需 **hedera_dual 式 per-request 路由**(/wallet→REST body, eth_*→jsonrpc), "复用 hedera_dual.py 现成范式, 不新造, 按链非按 family"。**∴ tron 不是补 PRESET, 是 S3.7 per-request 路由改动**(与 polkadot 混协议一起做)。批3 收尾先做 near/ton/hedera 补配置, tron 归 S3.7。**【2026-06-05 已完成】tron family=hedera_dual 复用通用 dual 路由 + 补 /wallet body PRESET + contract_address business_id 池修复, 36/36 healthy。**
      - **🔴🔴 polkadot 混协议(批3 真正剩的最后一块, 2026-06-05 全量语义核对发现, 未完成)**: polkadot family=substrate **但混协议**: 3 substrate-jsonrpc(system_account/account_nextIndex/chain_getHeader) + 3 Sidecar REST(GET /accounts/{addr}/balance-info, GET /blocks/{n}, GET /pallets/staking/progress)。family=substrate 单一 → 3 REST method 走 substrate adapter(jsonrpc 构造器)**构造崩**(transport rest_path 不在 build_params_from_spec)。**∴ polkadot 需 dual 路由, 但路由规则与 hedera_dual/tron【相反方向】**: hedera/tron 是 "eth_*→jsonrpc 否则 rest"; polkadot 是 "path风格(/或GET开头)→rest 否则→substrate jsonrpc"。**∴ hedera_dual 的 _is_jsonrpc_method(只认 eth_*) 不能直接复用给 polkadot**。方案: (甲)扩展通用 dual 路由按"path风格分"覆盖 polkadot/tron/hedera; (乙)polkadot 单独 substrate_dual。**这是批3 最后未完成项, 不是"批3 完成了"**(我全量核对前误以为 36/36 = 完成, 实际 polkadot 3 REST method 在 mixed 里崩, test_10 只测 single=system_account 走 jsonrpc 没暴露)。
- **🔴 批4-a 已完成(2026-06-05 commit)**: mixed weight 加权 round-robin 真驱动(缺口#9)。config_loader get_current_rpc_methods mixed 分支重写读 mixed_weighted + sum==100 fail-fast + 按 weight 展开加权数组(weight=30→method 重复30次); target_generator round-robin 落在加权数组上=加权(逻辑零改, 可复现); 36/36 链补真实占比(A方案递减模板 4=40/25/20/15 5=30/25/20/15/10 6/7 类推, 全 sum=100, 使用者按真实业务调整)。实证频次==占比(ethereum/solana/cardano/base 实跑)。全门 R0/F2/ci_smoke 过。**批4 剩: 4-b _meta.adapter_family 保留(config_loader L597 del._meta) + 4-c fetch 多池真值(create_adapter 补bitcoin/UTXO + 写盘多池, 让F2 KNOWN-PENDING 32转healthy)**。
- **🔴 批4-b + 4-c-1 已完成(2026-06-05 commit)**: ① 4-b 缺口#4: config_loader 保留 _meta.adapter_family(原 del._meta 整剥, 改提取为顶层 _adapter_family 键, 下游 shape 不破坏) ② 4-c-1 缺口#3 fake-node/CI 路径(R-D, SSOT L58): param_spec.extract_pools_from_fixture 从 <method>.request.json 按 spec source 提真值分池(account/tx_hash/block_height/business_id), cli.py 接入填池替代池空 fail-fast。**F2 KNOWN-PENDING 32→8(healthy 152→176, 24 method 转 healthy, 真值非占位实证)**。**剩 8 个(avalanche-x avm.* *_encoding + cardano POST_TX_INFO/POST_BLOCK_TXS/POST_ASSET_INFO + tron triggerconstantcontract/gettransactionbyid)= param_format 缺 PRESET 映射(构造层缺口非供给层), resolve_param_spec fail-fast, 待补 PRESET + POST body 复杂构造(批3 PRESET 尾巴)**。**批4-c-2(真节点 fetch 抓多池写盘 + bitcoin/UTXO adapter)归真机 L3 窗口**。全门 R0/F2/ci_smoke 过。
- **🔴 批4-c-1 收尾完成(2026-06-05 commit): F2 KNOWN-PENDING 8→0(184/184 全 method 真值构造)**。补 avalanche-x 3 PRESET(height/txid/addresses_limit_encoding, jsonrpc_dict+encoding, _resolve_slot_value 加 wrap_array+business_id) + cardano 3/tron 2 rest_body(extract_pools_from_fixture rest_body 重写: body_template 占位按序填对池, 兼容有/无 http 包装两形态)。**🔴 token-level 实跑抓真 bug**: cardano POST_ASSET_INFO [[policy,asset_name]] 两值都填 policy(rest.py business 占位每个都取 idx=0)→ 修按出现顺序递增 idx(policy=池0 asset_name=池1)+replace 1次。**批4 全部 a/b/c-1 完成**(c-2 真节点 fetch 抓多池归真机 L3 窗口)。全门 R0/F2(184/184 KNOWN-PENDING=0)/ci_smoke/build-target 36/36。剩 L2 占位 3(eth_call address_with_options)=S2 老项非批4。
