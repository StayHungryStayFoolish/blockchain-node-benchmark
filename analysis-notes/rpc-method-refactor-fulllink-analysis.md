...
## 76. 第六十三轮: 主入口 blockchain_node_benchmark.sh RPC 数据流编排真读(L128-202)— 完整入口链 + qps_test_status 双写入点

### 76.1 prepare_benchmark_data(L131-185)= RPC method 数据流入口编排
- **L137-141 fetch 调用**: `fetch_active_accounts.py --output $ACCOUNTS_OUTPUT_FILE --count $ACCOUNT_COUNT --verbose` = **只传 --output+--count**(不传 method/输入类型)→ fetch 只产 account 文件。
- **L160-166 target_generator 调用**: `--accounts-file $ACCOUNTS_OUTPUT_FILE --rpc-mode --output-single --output-mixed` = account 文件喂 target_generator。
🎯 **完整入口数据流**: fetch(产 account)→ target_generator(account+method→TSV→cli.py→vegeta target 文件)→ master_qps(vegeta attack target 文件)→ proxy 插流量(Phase 0.5 已起)→ attribution 归因。

### 76.2 🎯 qps_test_status 双写入点确认(纠正 §67)
- **初始创建**: 主入口 start_monitoring_system **L195-197 `echo "running" > qps_test_status`**(benchmark 启动建)。
- **运行时更新**: master_qps execute_single_qps_test L791 `echo "running qps:$qps"`(每轮 QPS 更新, §67)。
- 两个写入点: 主入口建(running)+ master_qps 更新(running qps:N)。生命周期信号源头在主入口 L195, 不是之前以为的单一点。

### 76.3 S1 入口编排落点
fetch 调用(L138)只传 --output/--count → **S1 扩展 fetch 多池输出后, 主入口这个调用点也要传新参数**(如 --pools account,tx_hash,block 或新输出格式)。这是 S1 的主入口编排落点(除 fetch 内部 L817-819 + target_generator reader 外的第三处)。
...
## 78. 第六十五轮: 现有 config 字段复用决案(用户要求列复用清单+核查覆盖度)

### 78.1 36 链覆盖度核查(纠正"复用 health_probe"的乐观说法)
| 字段 | 覆盖 | 现状 |
|---|---|---|
| param_formats / proxy_extraction / rpc_methods.mixed_weighted | **36/36** | 全链有(weighted 已有 weight 值=1 但代码没读) |
| _meta.health_probe | 5/36(rest4+hedera) | 只少数链 |
| _meta.rest_paths | 6/36(rest5+hedera) | 只 REST/hedera |
| _meta.json_rpc_url | 1/36(hedera) | 仅 hedera |
| methods(fetch) | 8/36 | 仅 fetch 支持的 8 链 |
**诚实修正**: 我之前说"block_height 复用 health_probe", 但 health_probe 只 5/36, 其余 31 链(所有 jsonrpc/substrate/tendermint/bitcoin)没有 → 不是"现成全有"是"复用 schema 形态 + 推广填全"。

### 78.2 复用决案(详见 design 文档 §6.3)
- **零新增直接复用**: param_formats(升级)/ proxy_extraction(三端同源)/ **mixed_weighted(直接读驱动 weight, 决策B已定)**。
- **复用 schema + 推广**: health_probe/rest_paths/json_rpc_url(部分链有, 复用形态推广到全链)。
- **决策A(我判断待用户确认)**: 新增独立 block_height_spec(全36链), 复用 health_probe schema 基因, 两者共存互补(health_probe=健康连通 / block_height_spec=落后多少, 不同功能各一字段非 parallel-entry)。

### 78.3 关键收获(用户提醒的价值)
用户问"能直接引用现有 config 变量么 + 列出来确认" → 逼出**精确覆盖度核查**, 纠正了我"复用 health_probe"的乐观(只5/36)。**mixed_weighted 36/36 已有 weight 是最大收获: S2.4 零新增字段直接读**。复用前必查覆盖度, 不能凭"某字段存在"就说"可复用"(可能只少数链有)。


## 79. 第六十六轮: 🔴 health_probe 消费链核查(回答"哪个方案不留技术债")— 块高实为三套并存债, 不留债=单一声明收敛

### 触发: 用户"哪个方案不留技术债"。先查 health_probe 真实消费(不凭字段名判断)。

### 79.1 GREP-EVIDENCE: health_probe 真实消费链
- `_meta.health_probe` 只被 rest.py:115 health_check_request + hedera 委派消费。
- health_check_request → cli.py:114 cmd_health_probe(health-probe 子命令)。
- **cmd_health_probe / health-probe 子命令生产 shell 无 caller = dead path**(同 §59 parse-height)。
- **health_probe 的 parse_jq 就是提块高**(cardano `.[0].block_height` / aptos `.block_height` / tezos `.level`)→ **health_probe ≈ 块高提取(REST版), 不是纯健康连通**(纠正我之前"两者不同功能"判断)。

### 79.2 🔴🔴 真相: 块高/健康提取实为【三套并存】= 已有 parallel-entry 债
框架现在"从节点取块高"分散三处:
1. **Shell `get_block_height`**(common_functions, 8链 case)= 生产 live path。
2. **Python `parse_block_height`**(各 adapter, 36 family)= 生产 dead path(§59)。
3. **`_meta.health_probe`(5链)+ Python health_check_request** = 生产 dead path(本轮)。
**三处本质都做"从节点取块高", 已是三套并存技术债。**

### 79.3 🎯 回答"哪个方案不留技术债"
- **新造 block_height_spec 当第4套 = 加重债(变4套)** ❌
- **不留债方案 = block_height_spec 作【单一声明源】, 让现有三套全部收敛读它** ✅:
  - Shell get_block_height 读 block_height_spec(替换 8链 case)。
  - Python parse_block_height 读同一 block_height_spec(替换硬编码, 或保留作测试夹具读同源)。
  - health_probe **收编进 block_height_spec 或明确降级**(它本就 dead path, 5链 REST 的块高提取并入 block_height_spec 的 REST 策略)。
- **即 block_height_spec 不是"第4个字段", 是"统一现有三套的单一真理源"**。这才是不留债(消除 3 套并存, 收敛为 1 声明 + N 消费者读同源)。

### 79.4 决策A 修正(基于消费链证据)
之前判断"新增独立 block_height_spec, 与 health_probe 共存互补"= **错(会变4套)**。修正: block_height_spec 作单一声明, **收编 health_probe**(REST 块高提取并入), get_block_height + parse_block_height + health_probe 三套全收敛读 block_height_spec。
- health_probe 现是 dead path(生产不调)+ 只5链 → 收编进 block_height_spec 无生产影响, 反而消除一套死字段债。
- **不留债的本质 = 单一声明源 + 所有消费者读同源, 不是"新造还是复用某一个现有字段"**(那是表面; 真问题是现有就有 3 套, 要收敛成 1)。

### 79.5 元教训
用户"哪个不留技术债"逼出 health_probe 消费链核查, 发现块高实为三套并存(不是"现有1套 vs 我新造1套"的二选一)。**判"复用还是新造"前必查现有字段的真实消费链 + 是否已有多套** —— 不查就答会漏掉"现有已是 parallel-entry"的真相, 把"4套收敛成1"误判成"1 vs 1 二选一"。


## 80. 第六十七轮: 🎯 块高功能真实目的 = 喂瓶颈检测(用户提醒"获取本地+网络高度是要做什么")— D5.1 必须保护的契约

### 触发: 用户反问"获取本地节点区块高度+主网已知最高高度是要做什么么"。回到目的检验设计, 别在字段收敛里跑偏。

### 80.1 🎯 块高功能真实目的(完整链, 代码实证)
**目的 = 判断被测节点在压测下是否跟得上链(节点掉队=瓶颈/失败信号)**, 喂给瓶颈检测:
```
本地高度 vs 网络已知最高 → block_height_diff → block_height_monitor 判 diff 持续超时(BLOCK_HEIGHT_TIME_THRESHOLD)
→ 写 block_height_time_exceeded.flag → bottleneck_detector 读 flag 判"节点是否被压垮" → 标记瓶颈/停 QPS 爬坡
```

### 80.2 bottleneck_detector 块高瓶颈判断核心(L1145-1214, 实证)
- **L1147 不直接读 block_height_diff, 读 `block_height_time_exceeded.flag`**(block_height_monitor 写的持续落后超时标志)。
- L1150-1157: flag==1 → is_node_critically_unhealthy=true。
- **4 场景**: A 瓶颈+健康(分RPC/资源)/ B 资源瓶颈+节点持续不健康=系统级真瓶颈 / **C 节点持续不健康(无资源瓶颈)=节点失败 Node_Unhealthy** / D 正常。
🎯 **节点跟不上链 = 独立瓶颈维度**(和 CPU/磁盘/网络并列), 甚至是"节点失败"判据(场景C)。这就是块高功能的业务价值, 不是孤立取数字。

### 80.3 🔴 D5.1 改造必须保护的契约(用户提醒的核心 = 别为解限流弄坏瓶颈判断)
D5.1 把 get_block_height 改"本地自查同步状态"后, **必须保证这条链不断**:
```
本地自查落后量 → block_height_monitor 判持续超时 → block_height_time_exceeded.flag → bottleneck_detector 4 场景
```
- **D5.1 本地自查产出的"落后量"语义必须能喂进 block_height_monitor 的"diff 持续超时"判断**(保持 block_height_diff 语义 = 节点落后网络多少)。
- 🎯 **D5.1 不仅没破坏反而强化**: 本地自查(substrate system_syncState / bitcoin getblockchaininfo 直接给本地 vs 网络已知最高)正是"节点是否跟上链"的直接答案, 比打外部主网可靠 —— **外部主网限流取不到时, 原方案这个瓶颈判断就瞎了**(取不到 mainnet_block_height → diff=N/A → flag 不触发 → 节点掉队也检测不到!)。本地自查解限流 = 块高瓶颈判断在任何链任何时候都能工作。
...
## 86. 第七十三轮: 系统资源图 vs per-method 图的逻辑关系(用户问"有什么逻辑关系")— 同源派生 + 守恒对账

### 86.1 🎯 关键确认: 两类图同源(同一份 performance CSV)
- 系统资源图: 用 `self.performance_csv`(report L1182, unified_monitor 产出的节点每秒总资源)。
- per-method 图: monitor_csv **fallback = self.performance_csv**(report L4277-4280 注释 "fall back to the performance_csv we already have")。
→ **两类图的"节点资源"是同一份 performance CSV, per-method 没有独立采集资源, 复用系统图同一份数据。**

### 86.2 🎯 逻辑关系: 同源派生(per-method = 系统资源 × method 流量占比)
```
unified_monitor → performance CSV(节点每秒总 CPU/MEM/磁盘/网络)
   ├── 系统资源图(直接画总量)         "节点用了60% CPU"
   └── per-method 图(总量×method流量占比拆分)  "getBlock占18%, getBalance占42%"
              ↑ proxy CSV(每method每秒请求数)提供拆分权重
```
1. **同源**: 两类图的节点资源 = 同一份 performance CSV(per-method 复用, 不独立采集)。
2. **派生**: per-method 图 = 系统资源数据 × 每method流量占比(proxy CSV)。per-method 是系统资源的"细分视角", 不是新数据。
3. **守恒对账**: per-method 各 method 资源之和 ≈ 系统总量(weight 之和=1)。某秒所有 method 归因 CPU 加起来 = 该秒节点总 CPU(60%×0.3+60%×0.7=60%)。= 它俩的数学关系 + 对账校验点。

### 86.3 为什么要两类图(互补非重复)
- **系统资源图**: "节点这个 QPS 下整体扛不扛得住"(找瓶颈: 总 CPU/磁盘/网络到没到天花板)。
- **per-method 图**: "哪个 method 在吃资源"(优化方向: getBlock 占 60% CPU 则优化它最划算)。
- **互补**: 系统图定"节点容量边界", per-method 图定"资源消耗归属"。benchmark 既要测最大 QPS(系统图)又要按 method 归因(per-method, NS-2)。

### 86.4 🎯 逻辑关系对重构的意义
- **S3.3 补四维低风险再确认**: per-method 加 EBS/Net = 从同一份 performance CSV 多读 disk/net 列 × weight(系统图已在画这些, per-method 只是按 method 拆)→ 数据现成只换切法。
- **出图机制应统一(强化 §83)**: 既然同源同性质数据(派生关系), per-method 用 SVG / 系统图用 matplotlib 的割裂无道理 → 强化"per-method 统一到 matplotlib + UnifiedChartStyle"决策。
- **守恒对账 = L3 验证项**: per-method 各 method 资源之和应 ≈ 系统总量(同一秒), 可作 S3.3 正确性断言(若不守恒说明 weight 或读列有 bug)。

### 86.5 用户连续3问(图区别→怎么统计→逻辑关系)的累积价值
逐步逼清 per-method 资源图的完整本质: **同一份节点资源数据(系统图)+ proxy 流量占比权重 → 按 method 拆分(per-method 图)**。这是 NS-2 per-method 归因的数据模型全貌, 也确认了 S3.3 四维补全是"同源换切法"(低风险)+ 出图须统一体系(避割裂债)。


## 87. 第七十四轮: "不同 method/mixed 组合看资源消耗"能力真实状态(用户好奇 NS-2 核心承诺)

### 用户好奇 = NS-2 北极星核心承诺的验收: 换不同 method/组合, 资源图能否反映各自消耗差异?

### 87.1 ✅ 能力存在(代码层), 但维度不全 + 产物层未验证(诚实分层)
**代码层已落地(读透确认)**:
| 能看到 | 状态 |
|---|---|
| 每 method QPS / p50p99延迟 / 错误率 | ✅ per_method_qps/latency/error_rate 图 |
| 每 method **CPU 消耗**(流量占比分摊) | ✅ per_method_resource 图(只 CPU) |
| 每 method **MEM 消耗** | ❌ attribution 算了 mem 但出图只画 CPU(per_method_charts L241 只读 cpu_pct)|
| 每 method **磁盘(EBS)/ 网络** | ❌ 完全没有(缺口#8) |
→ **现状只 CPU 一维真出图**, NS-2 要的四维(CPU/MEM/EBS/Net)缺 3 维。S3.3 补全。

### 87.2 single vs mixed 两套并存(代码层)
report L4312-4316: 产物文件名带 `_single`/`_mixed` 后缀, 两套数据/图并存可对照。
- **single 模式**: 单 method 纯压 → 看"纯 getBalance 压测下节点资源全貌"(无混合干扰)。
- **mixed 模式**: 多 method 混压 → 看"混合下各 method 资源占比"(流量占比分摊)。

### 87.3 三种对比场景的支持度(诚实)
| 场景 | 支持度 |
|---|---|
| (A) 同一 mixed 内各 method 资源对比(谁吃得多) | ✅ 支持(CPU; S3.3 后四维) |
| (C) single vs mixed 对比(同 method 单压 vs 混压差异) | ✅ 两套数据并存可对照 |
| (B) 不同 mixed 配比之间对比(配比A vs B 哪个省资源) | ❌ 不直接支持(框架一次跑一个配置, 要跑两次手动比)— 潜在新需求 |

### 87.4 🔴 诚实分层(探针纪律 §13.1 + 产物验证纪律)
- **代码层**: 上述能力的代码逻辑读透确认(attribution 算 + charts 出 + report 引)。
- **产物层未验证**: 我**没确认实际跑出来的产物里两套图真在、真能对照、数值真对**(无真节点 + fake-node 数据退化 §7 的历史)。按用户纪律"确认拿到预期产物≠确认代码逻辑通", 这是代码层结论, 不是产物层验证。
- **诚实**: "能看到不同 method 资源消耗"= 代码层成立(CPU 维)+ 设计层承诺(四维, S3.3 补)+ 产物层待真机/fake-node 数据丰富后验证。

### 87.5 对重构/验收的意义
- NS-2 核心承诺(按 method 看资源)= **CPU 维已落地, MEM/EBS/Net 待 S3.3 补**。这是 RPC 重构 S3.3 的直接价值点。
- **S3.3 完成后用户能看到**: 每 method 在 single/mixed 下的 CPU/MEM/EBS/Net 四维消耗 + 守恒对账(各 method 和≈系统总)。
- **潜在新需求(B)**: 不同 mixed 配比对比, 当前要手动跑两次。若用户要自动对比, 是 RPC 重构之外的新功能(可记 OQ)。
- **L3 验收**: S3.3 后必须真跑(真节点或 fake-node 丰富数据)产出四维 per-method 图 + 验证守恒, 才算 NS-2 这块"产物层"通(不能只代码层)。


## 88. 第七十五轮(实施前核对): 关键落点行号/计数 GREP-EVIDENCE 核对 — 抓 2 处文档不准

### 触发: 用户"开始考虑执行代码更新, 执行前再确认一次"。探针 = "我马上照文档改代码, 落点准不准"(74 轮累积文档可能有行号/计数偏差)。

### 88.1 ✅ 核对准确的落点(文档=代码实际)
- S3.1 id=1: bitcoin 40/67, jsonrpc 42/104, substrate 29/49, tendermint 39/62 ✅
- S2.4: config_loader L540/626 ✅; S3.5: common L180 get_block_height ✅, config L597 del._meta ✅
- S3.3: PerMethodResourceRow L63-68(cpu_pct/mem_mb)✅
- S1: fetch L802 sigs ✅, L814 most_common ✅, L819 f.write(account)✅
- S3.6: handler statusRecorder L103-111 ✅, maxBody L25/29/57-58(LimitReader 请求体限制, 响应 tee 参照)✅
- rest.go RequestID L74 ✅; target_generator round-robin L260, TSV L247/262 ✅

### 88.2 🔴 抓到 2 处文档不准(实施前价值 = 避免改错/漏改)
1. **S3.1 是 8 处 id=1 不是 9 处**: 实际 bitcoin/jsonrpc/substrate/tendermint 各 2 处 = 8。文档早期说"9 处"是误算(rest/hedera 走 _vegeta_post_json 不在 body 写 id, 不在此列)。**已修正 S3.1 为 8 处**。若不修, 实施时会找不存在的"第 9 处 id"。
2. **S2.4 weight 漏了 config_loader L674**: grep 发现取 rpc_methods 实为 **3 处**(L540 validate / L626 generate_auto_config / **L674 get_current_rpc_methods**), 文档 S2.4 只写 L626/L540 漏 L674。**已补**。若不补, 实施时漏改 L674 → weight 仍不生效(get_current_rpc_methods fallback 路径还取 mixed 逗号串)。

### 88.3 实施前核对结论
74 轮文档落点行号**绝大部分准确**, 抓到 2 处(8 vs 9 处 id / S2.4 漏 L674)= 实施前确认的直接价值(改代码前修正, 避免漏改/找错)。**其余标了行号的落点经 GREP-EVIDENCE 核对与代码一致, 可作实施依据。**
- 元教训: 长周期(74 轮)累积的文档, 行号/计数会有偏差(代码可能变 / 早期误算)。**实施前必须 GREP-EVIDENCE 重核所有关键落点行号**, 不能直接拿 N 轮前记的行号改代码。


## 89. 第七十六轮(实施前关键修正): parse_block_height 不是死代码=ABC设计契约/parallel-entry半成品(用户点破) + 出图主用 report_generator

### ⚠️ 用户点破我的危险误判: "没调用"≠"死代码", 要查清"是废弃 vs 设计好但漏集成"
我之前(§59/§79)说 parse_block_height/health_probe 是"生产 dead path 可收编"。**用户问"另外2个到底在做什么? 没调用是因为重构中符合设计但没集成到调用链么?"** → 查清后发现我判错了。

### 89.1 🔴 parse_block_height = ChainAdapter ABC 的 @abstractmethod 设计契约(非死代码)
base.py 实证: `parse_block_height` 是 ChainAdapter ABC **3 个 @abstractmethod 之一**(L53-54), 与 build_vegeta_target(L34)/ health_check_request(L44)并列。每个 family adapter **必须实现**。
🎯 **真相 = parallel-entry 经典形态(用户点破)**: chain_adapters(Python, 36链, 设计完整的 ABC 体系)是**重构到一半、设计好但主流程还没切过去的新体系**; Shell get_block_height(8链 case)是**旧体系还在生产用**。两套并存, 新的没接进主流程 → 所以 parse_block_height"没被调用"不是废弃, 是**新体系还没集成到生产调用链**。

### 89.2 🔴 重大方向修正: 不是"删 Python 那套", 是"主流程从旧 Shell 切到新 Python adapter 体系"
- 我之前"块高三套收编, Python 是 dead path 可收编/降级"= **错**(把目标态新体系当死代码)。
- **正解**: chain_adapters Python 体系是**为 36 链零代码加链设计的目标态**(ABC 契约完整), Shell get_block_height 8链case 是**待替换旧态**。块高归一方向 = **主流程切到 Python adapter 体系(36链)** 或 Shell 读与 Python 同源的声明 —— 不是删 Python。
- ⚠️ 这改变决策 A: block_height_spec 收敛, 但**保留并集成 chain_adapters parse_block_height**(它是 36 链目标态), 收编的是"旧 Shell 8链 case", 不是 Python。health_probe(rest 5链)是否也是半成品需同样查(它走 health_check_request ABC, 同理可能是目标态非死代码)。

### 89.3 ✅ 出图框架主用 report_generator(matplotlib)— 回答用户②
主入口 Phase 6/7(blockchain_node_benchmark.sh L710-737)真正调:
- **report_generator.py(L715/723)= 主出图模块**(双语 HTML + matplotlib)。
- advanced_chart_generator.py(L737)= 补充图表。
- **per_method_charts.py(SVG)是 report_generator 内部 import 调用**(§81 _generate_per_method_section_safe), 非主入口直接调。
🎯 **框架主出图 = matplotlib(report_generator)**, per_method_charts SVG 是体系里唯一 SVG 异类 → 确认用户拍板"统一 matplotlib"正确(per_method 归一到主体系)。

### 89.4 🔴 元教训(用户两次点破的同一类错)
- §66 我用 grep-shallow 冒充精读; 本轮我把"设计好但漏集成的新体系"(parse_block_height ABC)误判成"死代码"。**两次都是没查清就下判断**。
