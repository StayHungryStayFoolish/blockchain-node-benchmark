...
→ **已合入 NORTH-STAR §3 Q4-7**(2026-05-27 用户决策):
- 算法 = 加权 group_by(秒级时间窗)
- **权重源 = 公开资料先配粗粒度**(本仓库 `01-06` 研究档已有的 method "典型延迟量级" → 映射 1/10/100 三档)
- **后期实际使用根据真实压测数据迭代调整**(权重不是阻塞决策,先跑起来再调)
- PoC 撤销条件:ground truth 误差 > 20% **才**升级 (b) 加权回归;否则不升级

**实施含义**:阶段 4 PoC solana 不需要先做"完美 weight 表"才能跑,**先用粗粒度 1/10/100 三档跑起来,跑完看误差再调**。这避免了"调研完美主义"陷阱(为了 5% 精度多花 1 周调 weight,不如先验机制可行)。
