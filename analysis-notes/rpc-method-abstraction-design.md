...
- **S3.1 重建 request_id 关联键**(缺口#5, §53.1 GREP-EVIDENCE 精化落点): **真实落点 = 4 family adapter 构造 body 硬编码 `"id": 1` 共 9 处**(bitcoin_jsonrpc.py:40/67, jsonrpc.py:42/104, substrate.py:29/49, tendermint.py:39/62), **非 base.py _vegeta_post_json**(它只序列化不碰 id)→ 改为唯一 id(生成逻辑放 base.py helper 各 family 调)→ proxy extractor 已正确提取(jsonrpc.go:88 stringifyID; **rest.go:74 RequestID="" 需补**)→ 响应按 id 关联回 method。**⚠️ proxy 生命周期约束(§57 遗漏补全)**: proxy 由 `lib/proxy_lifecycle.sh` 在主入口 **Phase 0.5(Phase 1 前)启动**(L1103, P0-3 修复后时序), 启动时 `-chain=$chain_file` 加载 chain template 的 method 识别规则。S3.1 改动**绝不能动 Phase 0.5 启动时序**(否则复发"vegeta 绕过 proxy→per-method 数据稀疏"bug); 且要保持 proxy 失败时的静默降级路径(non-fatal, proxy_lifecycle.sh:131/188)+ 僵尸 reap(L68)。
