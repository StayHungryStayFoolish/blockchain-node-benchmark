...
## 19. 🎯 K8s 监控补全 — 无债方案定案(2026-06-01, 多轮扩大分析 + 真机验证收口)

> 触发: 用户要求"不留技术债的方案", 并反复要求"再扩大分析范围"——每轮都挖出真问题。
> 经 4 重确认: skill(parallel-entry K1-K7) + 全 6 条调用链 + 反向验证 + 目标发现链。
> 关键转折: 否决了最初的"选 b 单点改"和 A/B 二选一(都留债), 挖出第三条无债路。

### 19.1 多轮分析挖出的真问题(每轮一个"惊喜", 都已纳入)
1. **选 b 单点改 unified 循环条件 = AP3 caller-blind**: qps_test_status 是【整个监控子系统】统一
   生命周期信号(unified L2328 / disk_bottleneck L301 / ena L168 / block_height L413 / coordinator L508
   全靠它)。单改 unified 会让其它组件在 k8s 哑掉。→ 否决。
2. **直接复用 start_monitoring_system 会拉起 block_height**: 它连 LOCAL_RPC_URL/MAINNET_RPC_URL 取块高,
   DaemonSet 纯监控无 RPC 节点 → 该组件不该启。coordinator start_all 硬编码 5 组件无子集机制。
3. **🔴 目标身份错位(最根本)**: k8s_device_resolver 用 POD_NAME=自己(DaemonSet pod), cgroup_collector
   TARGET_PID 默认 self → 都在监控【采集 pod 自己】不是区块链节点 pod。之前"真机验证"用 `-p bench-node-sim`
   显式传对了 pod 名, 掩盖了生产输入(自己)是错的(parallel-entry K2 变体: 验证路径≠生产路径输入)。
4. **A/B 采集模型二选一都留债**: A(node级)依赖"独占 node"隐性契约债; B(cgroup级)磁盘只有累计 counter
   无 %util(cgroup 层物理拿不到)是能力残缺债。
5. **TMP_DIR 依赖链**(已证安全): TMP_DIR=CURRENT_TEST_DIR/tmp=DATA_DIR/current/tmp 确定性, 各组件各自
   source config_loader 算同值 → 信号可达, 非断链。
6. **DATA_DIR 可写性**: 默认 ${HOME}/blockchain-node-benchmark-result, DaemonSet 须 BLOCKCHAIN_BENCHMARK_DATA_DIR
   指向 pod 内可写卷(emptyDir)。

### 19.2 🎯 无债方案核心洞察: 每种资源用最干净的源, 都精确到目标 pod, 不依赖独占
**关键发现(真机验证)**: iostat 对每个块设备独立报 util/iops/throughput(GKE 实测 sda util2.86 / sdb util89
/ sdc util80 同刻三设备完全独立)。所以**磁盘隔离不靠"独占 node"也不靠"cgroup 切片", 靠"按设备名采"**
——一个 PVC=一个块设备, iostat 天然按设备隔离。这是块设备层比 cgroup 层和 node 级都干净的地方, 是无债的钥匙。

| 资源 | 无债采法 | 隔离方式 | util/速率 | 不依赖独占? |
|---|---|---|---|---|
| 磁盘 | iostat 采目标 pod 的 PVC 设备(sdb/sdc) | 块设备天然隔离 | ✅ 完整 util/iops/throughput | ✅(按设备不掺别的 pod) |
| CPU/MEM | cgroup_collector TARGET_PID=节点进程PID | cgroup 按 pod 切片 | ✅ 含 throttled(k8s 特有) | ✅(按 cgroup 切片) |
| network | hostNetwork ethtool(节点 pod 用 hostNetwork) | node 网卡=pod 网卡 | ✅ | ✅(节点 pod hostNetwork) |

### 19.3 目标发现链(全链路真机验证, 每环代码事实)
```
BLOCKCHAIN_PROCESS_NAMES (config_loader:23 既有)
  → pgrep -f (node hostPID 视角; unified_monitor:736 既有 pgrep BLOCKCHAIN_PROCESS_NAMES 逻辑)
  → 节点进程 PID (真机实测 fio PID=10131)
  → /host/proc/PID/cgroup (真机实测含 pod UID:
      0::/kubepods.slice/.../kubepods-besteffort-podd2697581_06f2_4ea2_a5fb_3cbde9def49a.slice/...)
  → pod UID (下划线转连字符 = d2697581-06f2-4ea2-a5fb-3cbde9def49a)
  → k8s API list pods on node + 匹配 UID (k8s_api_client:281 list_namespaced_pods(node_name) 既有)
  → pod name/namespace
  → pod_device_mapper.map_pod_volumes (解析该 pod PVC → sdb/sdc; 真机验证双盘 data→sdb/accounts→sdc)
  → iostat 采 sdb/sdc (真机验证 util/iops/throughput)
  + cgroup_collector TARGET_PID=10131 (resolve_target_cgroup:150 既有, 采该 pod cgroup cpu/mem)
```
**大部分零件框架已有**(pgrep 逻辑/list pods on node/resolve_target_cgroup/pod_device_mapper),
缺的只是"把它们串成一条自动发现链 + 修掉默认 self/自己的错"。

### 19.4 无债论证(逐条对照用户的债标准)
- ✅ 不依赖独占 node(磁盘按设备隔离 + cpu/mem 按 cgroup 切片 + network hostNetwork)→ 无隐性契约债
- ✅ 磁盘有 %util(走 iostat 块设备层非 cgroup)→ 无能力残缺债
- ✅ 统一信号 qps_test_status(所有组件一致, 不单点改)→ 无 caller-blind
- ✅ 目标发现复用框架既有 BLOCKCHAIN_PROCESS_NAMES 进程名机制 → 不发明新约定
- ✅ 监控目标精确(节点进程 PID/pod, 非采集 pod 自己)→ 无采错对象的静默错数据
- ✅ VM 路径不碰(k8s* 分支隔离)→ 无双套漂移
- ✅ block_height 不启(不连 RPC); ena 自动跳(GCP)→ 无不该跑的组件

### 19.5 实施清单(待动手)
代码:
1. 新增"目标发现"模块(config 或 monitoring): k8s* 时按 BLOCKCHAIN_PROCESS_NAMES pgrep(hostPID)→节点PID
   →/host/proc/PID/cgroup 提 pod UID→k8s API 匹配 pod name→导出 TARGET_PID + TARGET_POD_NAME/NAMESPACE。
2. 改 k8s_device_resolver: 用上面发现的 TARGET_POD_NAME(非 POD_NAME=自己)解析设备。
3. cgroup_collector: 用 TARGET_PID(上面发现的节点PID, 非 self)。
4. coordinator start_all_monitors: k8s* 子集=(unified, network, disk_bottleneck), 排除 block_height; VM 不变。
5. DaemonSet command: source config_loader→目标发现→建 qps_test_status→coordinator start→trap SIGTERM 删信号。
6. CSV rotation(无限运行防爆盘) + 镜像加 sysstat/ethtool/bc/jq/net-tools(K4) + BLOCKCHAIN_BENCHMARK_DATA_DIR
   指 emptyDir(DATA_DIR 可写) + hostNetwork:true + DEPLOYMENT_MODE=k8s_gke。
真机验证(GKE bench-k8s-test, 已就绪):
7. 端到端: DaemonSet 部署 → 目标发现找到 bench-node-sim → 采 sdb/sdc util/iops → performance CSV disk 列
   非空 → HTML 出图。network ethtool hostNetwork 采到。

### 19.6 待修正的已落地代码(§18.7 的 k8s_device_resolver 有 bug)
...
## 21. 🧱 K8s 运行环境硬约束地基(2026-06-01, 决定性事实, 新会话从这里出发重新设计)

> ⚠️ **本节是 §18/§19/§20 的地基订正。前面 N 轮(含 §20"定案")反复横跳, 根因 = 没先建立 GKE 运行
> 环境硬约束就开始设计。本节锁定硬约束, 新会话必须先吃透这些事实再设计, 不要再在"node裸跑 vs 容器化"
> "DaemonSet vs Job"之间横跳——硬约束已经把可行解空间缩小了。**

### 21.1 🔴 决定性硬事实(真机 SSH 验证, 这是地基)
1. **GKE node = Container-Optimized OS(COS)**: SSH 进 GKE node 实测——**没有 iostat/mpstat/sysstat**,
   COS 是**只读根文件系统 + 无包管理器(无 apt)**, 装不了工具。有 ethtool, 能 lsblk 看到 sda/sdb/sdc。
   → **benchmark 主流程(依赖 sysstat/bc/jq/python3/vegeta)不能在 GKE node 上裸跑**。
   → **必须容器化**(工具打进镜像)。这一条推翻"node 上整体跑 benchmark"(§20 倾向过的方向)。
2. **pod 容器内 cgroup 拿不到磁盘 %util**(cgroup 只有累计 counter)→ 需 node 级 iostat。
3. **node 级 iostat 需容器 hostPath 挂 /proc /sys + privileged**——这正是 deploy/k8s/04-daemonset.yaml
   已具备的配置(hostPID/privileged/hostPath /host/{proc,sys,dev})。
4. **一个 node 独占一个区块链节点 pod** → node 级资源 ≈ 该 pod 资源(因独占)。
5. **框架是有界 benchmark**(QPS 爬坡到 MAX_QPS 或瓶颈 AUTO_STOP → 出 HTML 报告), 非无界 daemon。
6. **监控采集是 benchmark 主流程的 Phase 2**(blockchain_node_benchmark.sh: Phase0.5 proxy→1 prepare→
   2 start_monitoring_system 起后台采集→3 QPS压测→停→报告), 监控与 QPS 压测同生命周期(qps_test_status 信号)。
   → 监控不是独立常驻的东西, 是 benchmark 的一个阶段。

### 21.2 硬约束缩小后的可行解空间
COS 强制容器化(事实1) + node 级 iostat 需 hostPath 容器(事实3) → **必然是某种特权容器**。
但容器跑"有界 benchmark 全流程"还是"只采集", 是核心待定(见 21.3)。
- DaemonSet(常驻)= §19/§20 反复争的载体。它配置(hostPath/privileged)对, 但"常驻"跟"有界 benchmark"(事实5)
  + "监控是 Phase 2 子阶段"(事实6)矛盾——常驻采集器跟 benchmark 主流程的 Phase2 内置监控会是【两套监控】。
- 可能更对的形态: **一个特权 Pod/Job(非 DaemonSet), 调度到节点 pod 所在 node, 容器内跑完整
  blockchain_node_benchmark.sh 有界流程**(Phase0.5→报告), 容器有 hostPath 挂 /host 所以 Phase2 监控能采
  node 级 iostat(=节点 pod 磁盘, 因独占), vegeta 打节点 pod 的 RPC(hostNetwork → node 网络可达)。
  这样: 一套监控(Phase2 内置)、有界(跑完出报告)、容器化(COS 兼容)、node 级 iostat(hostPath)。
  **此形态 §20 否过(当时以为 node 裸跑), 但 COS 事实下"容器内跑全流程"才是它的正确版本。待新会话验证。**

### 21.3 新会话必须重新分析/验证的核心(带具体验证点)
1. **vegeta 怎么打节点 pod 的 RPC**: 容器内跑主流程, LOCAL_RPC_URL 指向节点 pod。节点 pod hostNetwork
   → RPC 端口在 node 网络。容器(同 node, hostNetwork 或 hostPath)能否 localhost:port 打到? 真机验。
2. **容器形态: Pod/Job(有界, 跑完退) vs DaemonSet(常驻)**: 基于事实5/6, 倾向 Pod/Job 跑全流程, 但要验
   blockchain_node_benchmark.sh 在特权容器内能否整体跑通(它的 Phase2 监控在容器内 + hostPath 能否采 node iostat)。
3. **现有 deploy/k8s/(DaemonSet+cgroup_collector)怎么处置**: 若改成 Pod/Job 跑全流程, DaemonSet 那套是
   独立子功能(cgroup 采集), 跟全流程关系要厘清(保留为可选? 废弃? 还是全流程容器复用其 hostPath 配置)。
4. **镜像**: 打含 sysstat/ethtool/bc/jq/net-tools/python3/vegeta + 框架代码的镜像(K4 教训), push AR。
5. **目标定位**: 独占前提下用户手配 LEDGER_DEVICE(node 上 lsblk 看 sdb/sdc)最简; §18.7 的 device_resolver
   POD_NAME bug 在"容器内跑全流程"下要重新评估(可能根本不需要它)。

### 21.4 本会话教训(给新会话避坑)
- **根因**: 没先建立运行环境硬约束(COS 只读/缺工具)就开始设计 → 在"node裸跑 vs 容器化""DaemonSet vs Job"
  "node级 vs cgroup级"之间反复横跳 8+ 轮, 每轮推翻前轮。**新会话先锚定 §21.1 六条硬事实, 再设计。**
- §18/§19/§20 都有基于不完整前提的弯路结论。**以 §21 地基为准**, 前面章节当"分析过程"读, 不当"定案"。
...
## 29. ✅ config 治理 第3组完成(B/C/E/契约注释) + 🔴 发现独立 bug: ENA-on-GCP(2026-06-01)
### 29.1 第3组改动(加注释/统一默认, 非加 :-)
- **E 类 MONITOR_INTERVAL 默认值统一**: 下游 5 处 fallback `:-10` → `:-5`(与 user_config 默认 + coordinator 一致)。
  ena_network_monitor.sh:20/151, bottleneck_detector.sh:22, unified_event_manager.sh:12, network_monitor.sh:26/57。全仓无 :-10 残留。
- **B 类 ENA_MONITOR_ENABLED 注释**: 说明由 provider 决定(见下 bug)。
- **C 类 VOL_MAX_THROUGHPUT 注释**: io2 盘时由 IOPS 自动算覆盖, 手配仅非-io2 生效。
...
- 第2组 A 类 47 配置项: env 化 + VM/k8s 双验(commit 125f07f)
