...
- v1.2 (2026-05-19): 用户 YYYY 确认，新增三大章节：
  ...
  - 新增 `analysis-notes/research_notes/` 三份调研报告（01 EVM / 02 Solana-Sui-Aptos / 03 Bitcoin-Starknet)[**注:02/03 部分内容 out-of-scope — Aptos/Bitcoin 不在 baseline 真 8 链;v1.4.1 重写为 02-solana-sui-rpc + 03-evm-l2-rpc 覆盖 scroll/polygon**]
...
## §A.5.6 真 8 链清单铁律 (v1.4.1 校正, 2026-05-20)

**触发事件**:S0.2 验证发现 plan v1.3/v1.4 全文 11+ 处用 "Bitcoin + Aptos" 当 8 链清单成员,
事实上 baseline 真 8 链不含 Bitcoin/Aptos,而是 Scroll/Polygon。

**真相来源**(双重交叉验证):
1. `config/config_loader.sh:660` `supported_blockchains=("solana" "ethereum" "bsc" "base" "scroll" "polygon" "starknet" "sui")`
2. `tools/mock_rpc_server.py:387` `CHAIN_HANDLERS` 映射:8 链 → 4 个 handler

**真 8 链 RPC method 矩阵**(S0.2 实测 8/8 mock smoke 全过):

| # | 链 | chain_type | handler | mock port | 探测 method | 返回示例 |
|---|----|-----------|---------|-----------|-------------|----------|
| 1 | Solana | solana | handle_solana (74 行) | 18800 | getSlot | 1779352278 |
| 2 | Ethereum | ethereum | handle_evm (215 行,共用) | 18801 | eth_blockNumber | 0x8d6903c |
| 3 | BSC | bsc | handle_evm (共用) | 18802 | eth_blockNumber | 0x8d6903c |
| 4 | Base | base | handle_evm (共用) | 18803 | eth_blockNumber | 0x8d6903c |
| 5 | Scroll | scroll | handle_evm (共用) | 18804 | eth_blockNumber | 0x8d6903c |
| 6 | Polygon | polygon | handle_evm (共用) | 18805 | eth_blockNumber | 0x8d6903c |
| 7 | Starknet | starknet | handle_starknet (8 method) | 18806 | starknet_blockNumber | 148279356 |
| 8 | Sui | sui | handle_sui (7 method) | 18807 | sui_getLatestCheckpointSequenceNumber | 148279356 |

**rpc_methods.single/mixed (来自 `config_loader.sh` UNIFIED_BLOCKCHAIN_CONFIG)**:

| 链 | single | mixed |
|----|--------|-------|
| Solana | getAccountInfo | getAccountInfo,getBalance,getTokenAccountBalance,getRecentBlockhash,getBlockHeight |
| Ethereum/BSC/Base/Scroll/Polygon (5 EVM 共用) | eth_getBalance | eth_getBalance,eth_getTransactionCount,eth_blockNumber,eth_gasPrice |
| Starknet | starknet_getClassAt | starknet_getClassAt,starknet_getNonce,starknet_getStorageAt,starknet_blockNumber |
| Sui | suix_getOwnedObjects | suix_getOwnedObjects,suix_queryTransactionBlocks,sui_getLatestCheckpointSequenceNumber |

**铁律**:
1. 本 plan 任何后续段落引用 8 链时,必须以本表为准
2. 文档遗留的 Bitcoin/Aptos 已标 `~~OUT-OF-SCOPE~~` 删除线;不得在新文档/代码中复活
3. v1.4 文档遗留旧表(§11/§17/§18/§21)已就地标注校正,**新写代码必须读 `config/config_loader.sh:660`,严禁硬编码**
4. 加新链流程见 §18 chain-as-plugin(只改 JSON 不改代码,真要加 Optimism/Avalanche 走该流程)

**S0.2 验证脚本**:`tests/smoke_mock_rpc_8chains.sh` (1940 bytes,8/8 全过)
...
- Sui:客户常用 EKS + Karpenter(Aptos 不在 baseline 真 8 链,见 v1.4.1 §A.5.6 校正)
...
1. 写 `tools/mock_blockchain_node.py` (stdlib only, 无 PyPI 依赖)
   ...
   - 支持 8 链核心 method：
     ...
     - Sui/Starknet/Scroll/Polygon 已实现（详见 §A.5.6 真 8 链清单）
...
| Scroll | reth/geth | ⚠️ Geth `eth/*` Prometheus metric(无 per-method) | 中 |
| Polygon | bor/heimdall | ⚠️ Bor 类似 geth(无 per-method) | 中 |
...
**REST API 链特殊处理(Aptos)**[**OUT-OF-SCOPE — Aptos 不在 baseline 真 8 链**]:
~~Aptos 走 REST 而非 JSON-RPC,method 解析改为 URL 路径模板化:~~
```python
# /accounts/0xabc.../resource/0x1::coin::CoinStore → "GET /accounts/{addr}/resource/{type}"
```
~~URL 路径模板列表在 `config/chains/aptos.json` 的 `rest_paths` 字段定义。~~ **删除:Aptos 非 baseline 链**
...
对 Solana/Sui/Pathfinder/Juno 这 4 个暴露 per-method metric 的客户端:
...
对 Ethereum/BSC/Base/Scroll/Polygon 这 5 个无 per-method metric 的 EVM 链:
...
#### ~~Aptos (REST API)~~ [**OUT-OF-SCOPE — Aptos 不在 baseline 真 8 链**]
```json
"mixed": {
...
```
~~依据:Aptos Labs indexer 流量 > fullnode 的间接推断(fullnode 实际占比较低,但本表针对 fullnode benchmark 场景)。~~ **整节 OUT-OF-SCOPE,建议替换为 Scroll/Polygon (EVM L2,RPC method 与 ethereum 相同,参见 §A.5.6)**

#### ~~Bitcoin~~ [**OUT-OF-SCOPE — Bitcoin 不在 baseline 真 8 链**]
```json
"mixed": {
...
> **Sui / Starknet / Scroll / Polygon**:v1.3 baseline 阶段不引入复杂 fixture 池,由 chain config JSON 内的 `params` 直接给固定值,留 v1.4.1+ 扩展。**Aptos/Bitcoin 不在真 8 链,删除原条目**
...
加新链(如 Scroll 已是 baseline 链;若未来加 Optimism)只需写一份 `chains/optimism.json` + 落 `fixtures/optimism/*` 数据文件,**不改任何 .sh / .py 代码**。(原例用 Aptos,已修正为真链 Scroll + 假设 Optimism)
...
| ~~**Bitcoin**~~ | ~~OUT-OF-SCOPE~~ | ~~不在 baseline 真 8 链~~ | ~~见 v1.4.1 校正~~ |
| ~~`block_hashes.txt`~~ | ~~50~~ | ~~5 000–50 000~~ | ~~3 KB~~ |
| ~~`tx_ids.txt`~~ | ~~100~~ | ~~5 000–50 000~~ | ~~7 KB~~ |
| **Sui** |  |  |  |
| `object_pool.txt` | 50 | 2 000–10 000 | 4 KB |
| `tx_digests.txt` | 100 | 5 000–50 000 | 9 KB |
| **Starknet** |  |  |  |
